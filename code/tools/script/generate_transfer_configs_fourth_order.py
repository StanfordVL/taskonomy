...
if __name__=="__main__":
    ...
    configs = [] 
    for first_task,second_task,third_task,fourth_task in top_5_combinations:
    ...
    if args.target_task in ['ego_motion', 'fix_pose', 'point_match', 'non_fixated_pose'] \
        ...
        with open('/home/ubuntu/task-taxonomy-331b/tools/config_list_multi.txt', 'a') as fp:
            for config in configs:
                fp.write("fourth_order/DO_NOT_REPLACE_TARGET_DECODER/1k/%s\n" % config)

    with open('/home/ubuntu/task-taxonomy-331b/tools/config_list_fourth_order.txt', 'a') as fp:
        for config in configs:
            fp.write("%s\n" % config)
