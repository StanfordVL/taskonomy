...
segmentsemantic_rb class_places class_1000 alex lsm cmu pose5 pose6 poseMatch"
INTERMEDIATE_TASKS="autoencoder colorization curvature denoise edge2d edge3d \
fix_pose impainting_whole jigsaw keypoint2d keypoint3d \
non_fixated_pose point_match reshade rgb2depth rgb2mist rgb2sfnorm \
room_layout segment25d segment2d vanishing_point_well_defined \
segmentsemantic_rb class_places class_1000 class_selected alex lsm cmu pose5 pose6 poseMatch"
INTERMEDIATE_TASKS="random"
# DST tasks left out
...
export INSTANCE_TAG="final/${intermediate} --data-split train --places";
#export INSTANCE_TAG="${intermediate} --metric-only --from-scratch --layer 3 --dropout 0.3 --hidden 2048 --data-use 50000"
#export INSTANCE_TAG="${intermediate} --metric-only --train-encoder"
#export INSTANCE_TAG="${intermediate} --from-scratch --train-encoder --transfer-type full_taskonomy_beta1"
