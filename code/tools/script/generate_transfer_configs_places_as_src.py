...
if __name__=="__main__":
    ...
    prepend = '/home/ubuntu/task-taxonomy-331b/tools/config_list_places_affected_'
    if args.target_task == 'class_places':
        prepend = '/home/ubuntu/task-taxonomy-331b/tools/config_list_places_'

    top_5 = data[args.target_task][:6]
    top_5 = [x[0] for x in top_5]
    top_5_combinations = list(itertools.combinations(top_5, 4))
    configs = [] 
    for a_combo in top_5_combinations:
        if 'class_places' not in a_combo and args.target_task != 'class_places':
            continue
        first_task,second_task,third_task,fourth_task = a_combo 
        command = "python /home/ubuntu/task-taxonomy-331b/tools/script/generate_transfer_configs.py \\\n--prototype_path /home/ubuntu/task-taxonomy-331b/assets/transfers/config_prototype_data_check_second_order_no_image.py \\\n--input-task {first_task} --target-task {target_task} \\\n--hidden-size {hidden_size} \\\n--kernel-size {kernel_size} \\\n--num-layers {num_layer} \\\n--finetune-decoder False \\\n--retrain-decoder False \\\n--unlock-decoder True \\\n--target-decoder-func {target_decoder_func} \\\n--data-used {data_used} \\\n--second-task {second_task},{third_task},{fourth_task} \\\n--dest-dir /home/ubuntu/task-taxonomy-331b/experiments/fourth_order/{target_decoder_func}/{data_used}/\n".format(
        ...
        os.system(command)
        config_name = "{}__{},{},{}__{}__8__unlocked \\".format(first_task, second_task, third_task, fourth_task, args.target_task )
        configs.append(config_name)
    with open('{}fourth_order.txt'.format(prepend), 'a') as fp:
    ...
    for a_combo in top_5_combinations:
        if 'class_places' not in a_combo and args.target_task != 'class_places':
            continue
        first_task,second_task,third_task = a_combo 
        ...
        configs.append(config_name)
    with open('{}third_order.txt'.format(prepend), 'a') as fp:
        for config in configs:
            fp.write("%s\n" % config)
    ...
    top_5 = data[args.target_task][:5]
    top_5 = [x[0] for x in top_5]
    top_5_combinations = list(itertools.combinations(top_5, 2))
    configs = [] 
    for a_combo in top_5_combinations:
        if 'class_places' not in a_combo and args.target_task != 'class_places':
            continue
        first_task,second_task = a_combo 
        command = "python /home/ubuntu/task-taxonomy-331b/tools/script/generate_transfer_configs.py \\\n--prototype_path /home/ubuntu/task-taxonomy-331b/assets/transfers/config_prototype_data_check_second_order_no_image.py \\\n--input-task {first_task} --target-task {target_task} \\\n--hidden-size {hidden_size} \\\n--kernel-size {kernel_size} \\\n--num-layers {num_layer} \\\n--finetune-decoder False \\\n--retrain-decoder False \\\n--unlock-decoder True \\\n--target-decoder-func {target_decoder_func} \\\n--data-used {data_used} \\\n--second-task {second_task} \\\n--dest-dir /home/ubuntu/task-taxonomy-331b/experiments/second_order/{target_decoder_func}/{data_used}/\n".format(
                first_task=first_task,
                target_task=args.target_task,
                hidden_size=args.hidden_size,
                kernel_size=args.kernel_size,
                ...
                second_task=second_task)
        os.system(command)
        config_name = "{}__{}__{}__8__unlocked \\".format(first_task, second_task, args.target_task)
        configs.append(config_name)

    with open('{}second_order.txt'.format(prepend), 'a') as fp:
        for config in configs:
            fp.write("%s\n" % config)
